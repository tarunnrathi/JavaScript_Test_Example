...
import PodcastAjax from "components/Common/PodcastAjax";
import PodcastBody from "./podcast/PodcastBody";
import BreadcrumbCommon from "widgets/Common/Responsive/BreadcrumbCommon";

const PodcastDetails = (props) => {
  const { publicRuntimeConfig } = getConfig();
  const { isAjax = false, topPriorityData } = props;
  const [qp, setQp] = useState('');
  const [currentUrl, setCurrentUrl] = useState('');

  useEffect(() => {
    setCurrentUrl(topPriorityData?.currentUrl);
  }, []);

  const [category, setCategory] = useState('');
  const outBrainUrl = !isAjax
    ? currentUrl.replace(
 ...
 const query = topPriorityData?.query;
  useEffect(() => {
    if(query?.category) {
  ...
  const pageData = categoryDetailsData?.articleDetails
      ? categoryDetailsData?.articleDetails
      : "";

  // const categoriesList = categoryDetailsData?.categoryList;
  const podcastEmbed = pageData ? pageData?.podcast_embed : "";
  const articleBody = pageData ? pageData?.body : "";
  const tagsArray = pageData ? pageData?.tags : "";
  const headline = pageData ? pageData?.display_headline : "";
  const articleUrl = pageData ? pageData?.state_url : "";
  const photoStories = !isAjax ? topPriorityData?.photogallery.items : [];
  const webUrl = pageData? pageData?.weburl: "";
  const topStories = !isAjax ? topPriorityData?.topStories : [];
  ...
  useEffect(() => {
    ...
    const publishedByName = publishedBy.length ? publishedBy[0]?.english_name : "";
  ...
  const formattedDate = pageData?.updated_at
    ? dateFormat(pageData?.timestampUpdateDate, "mmmm d, yyyy, h:MM TT")
    : "";
  const intro = pageData ? pageData?.intro : "";
  ...
  const pageUrl = baseUrl+weburl_r;
  const palinHeadline = headline?.replace(/[^\w ]/, '');
  const bodyData = {
    intro: intro,
    ...
    <>
          ...
          <div className="podcast_container">
           ...
           {!isAjax &&
            ...
            <div className="podcast-listing_wrapper">
              <div className={isAjax? "podcast-listing_left_Ajax" : "podcast-listing_left"} id="podcast_div">
                <div
                  ref={ref}
                  className="podcast-consumption"
                  onMouseOver={() => {
                    setCurrentArticleId(storyId);
                  }}
                  rel="https://hindi.news18.com/podcast/podcast/jeevan-samvad-motivational-talks-by-dayashankar-mishra-about-corona-and-wedding-2-3345763.html"
                  galtitle="जीवन संवाद: कोरोना और शादी!"
                  id="podcast_consumption_3345763"
                  data-podcast_id="3345763"
                  data-author="News18 Hindi"
                  data-ga7="hindi.news18.com| unassigned |News18 Hindi"
                >
                <PodcastBody bodyData={bodyData} />
                {!isAjax && (
                  <LazyLoad once offset={100}>
                    <div id="infinite-scroll-section">
                      {stories && stories.length ? (
                        <PodcastAjax
                          key={"infinite"}
                          articles={stories}
                          pageAds={props.pageAds}
                          rhsPhoto={photoStories}
                          rhsTopStory={topStories}
                          urlParam={qp}
                          setCurrentArticle={props.setCurrentArticle}
                          comp={PodcastDetails}
                          cat={qp ? qp.category : "jeevan-samvad"}
                          isMobile={false}
                          dtype="news"
                          isCricketNext={false}
                        />
      ...
      <style jsx global>
        {`
          ...
          .podcast_breadcrumbs ul li a:hover{
            color: #e1261d
          }
          ...
          .podcast-consumption-content strong{
            font-weight:bold;
          }
