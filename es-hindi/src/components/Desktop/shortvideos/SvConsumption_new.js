...
export default function SvConsumptionDesktop({ data }) {
  const [showIcon, setShowIcon] = useState(false);
  const [activeIndex, setActiveIndex] = useState(0);
  const { vid_exist, vidStreamData, allData, articleData } = data;
  const cat =
    data?.category?.slug === "short-videos"
      ? ""
      : (data?.category?.slug || "") + "/";
  const shareUrl = getRelativeURL(false, articleData.weburl);
  ...
  const handleOnScroll = (action, currentIndex) => {
    if (action === "next") {
      const nextData =
        currentIndex + 1 < allData.length
          ? allData[currentIndex + 1]
          : null;
      const ele = document.getElementById(nextData?.story_id);
      ele.scrollIntoView({ behavior: "smooth" });
      setActiveIndex(currentIndex + 1);
      history.pushState(
        {},
        nextData.display_headline || "Short-Videos",
        nextData.weburl_r
      );
      logEvent("infi_scroll_vv", "play", nextData.weburl);
      setTimeout(() => {
        window.refreshPubstackPlayers(allData[currentIndex + 1]?.vidStreamData);
      }, 100);
    } else {
      const prevData = currentIndex - 1 >= 0 ? allData[currentIndex - 1] : null;
      const ele = document.getElementById(prevData?.story_id || null);
      ele.scrollIntoView({ behavior: "smooth" });
      setActiveIndex(currentIndex - 1);
      history.pushState(
        {},
        prevData.display_headline || "Short-Videos",
        prevData.weburl_r
      );
      logEvent("infi_scroll_vv", "play", prevData.weburl);
      setTimeout(() => {
        window.refreshPubstackPlayers(allData[currentIndex - 1]?.vidStreamData);
      }, 100);
    }
    ...
    <>
      <div className="vdwrapper">
        <div id="JsPlayer">
          {allData.map((video, index) => {
            ...
            const videoId =
              auto_youtube_import?.nw_auto_yt_feed_id || video_details.mongo_id || video_details.video_id;
            return (
              <div key={story_id} id={story_id} className="mainCanvas">
                {activeIndex === index && (
                  <div
                ...
                <div className="social-share">
                  {index !== 0 && (
                    <>
                      <span
                        onClick={() => handleOnScroll("prev", index)}
                        className="prevIcon"
                      >
                        <img
                          isLazyLoad={false}
                  ...
                  {index !== allData?.length - 1 && (
                    <>
                      <span
                        onClick={() => handleOnScroll("next", index)}
                        className="nextIcon"
                      >
                        <img
                          isLazyLoad={false}
                  ...
                  <a className={showIcon ? "show" : "hide"}
                  ...
                  <a className={showIcon ? "show" : "hide"}
                    href={`https://wa.me/?text='Short Video - ${shareUrl}`}
                    target="_blank"
                  >
                  ...
                  <a
                    ...
                    rel="nofollow"
                    alt={encodeURIComponent(display_headline)}
                  ><span className="telegram"></span></a>
                  ...
                  <a className="home_a" href="/" target="_blank">
      ...
      <style jsx global>
        {`
          ...
          #JsPlayer figurecaption h1.story-title span {
          ...
          #JsPlayer figurecaption h1.story-title .sit_cat {
            width: 4px;
            height: 4px;
            background: no-repeat padding-box #c4c4c4;
            vertical-align: middle;
          ...
          .social-share span.nextIcon {
            transform: scale(1.5);
            width: 32px;
            height: 32px;
            margin: 10px 0 3px 10px;
            }
            .social-share span.prevIcon{ transform: rotate(180deg) scale(1.5);width: 32px; height: 32px; margin: 0 0 3px 10px;
              display: inline-grid;}
           
          .social-share span.nextIcon img, .social-share span.prevIcon img {
              margin: 0;
              width: 32px;
          }
          a.back-ic {
