...
const VideosMobile = (props) => {
    ...
    const [loadMoreData, setLoadMoreData] = useState(props?.data?.latestVideosStories?.length > 0 ? props?.data?.latestVideosStories : 0);
    const [hideLoadMore, setHideLoadMore] = useState(false);
    const loadMore = async () => {
        let tempData = await getArticleList({
            count: 20,
            offset: loadMoreData?.length > 0 ? loadMoreData?.length : 0,
            filter: { post_type: "vodeos", 'tags.slug': `olympics-${olympics_year}` },
                        ...
                        <div className="CN-sec-l">
                            ...
                            <div className="CN-innersection">
                                <div className="cn-heading-1">
                                    <div className="headinner"><h1>OLYMPICS {olympics_year} <span>Videos</span></h1></div>
                                    <div className="icon" />
                                </div>
                                {/*CN-videoinner START----*/}
