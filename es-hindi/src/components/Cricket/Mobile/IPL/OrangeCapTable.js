import { displayAds, IPL_YEAR, ORANGE_CAP_HEADER } from "includes/ipl.helper";
...
const OrangeCapTableMobile = ({ data }) => {
  const { tableData=[], seoData, pageAds, ImageIdsAvailable } = data;
  const headerArr = ORANGE_CAP_HEADER;
  return (
    <>
        ...
        <BreadcrumbCommon breadCrumbArray={[
                  ...
                  { value: `ऑरेंज कैप ${IPL_YEAR}`},
                ]}
                isCapitalize={true}/>
        <div className="sponsor-with-heading">
            <h1 className="ipl-globahd">ऑरेंज कैप {IPL_YEAR}</h1>
...
export default OrangeCapTableMobile;