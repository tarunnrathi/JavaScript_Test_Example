...
const IplPhotos = ({ data }) => {
  const { photoPriority, dataLength, paramObj, pageAds, seoData } = data;
  // const page = paramObj.page;
  // const noContent = page > 24 ? false : true;
  const pageLimit = paramObj.pageLimit;
  const query_arr = { post_type:"photogallery",'tags.slug':`ipl-${IPL_YEAR}`};
  const {loadMore, categoryData, hasMoreData} = useLoadMore(photoPriority, pageLimit, dataLength, query_arr)
  ...
  return (
    <>
        ...
        <div className="sponsor-with-heading">
            <h1 className="ipl-globahd">IPL {IPL_YEAR} फोटोगैलरी</h1>
