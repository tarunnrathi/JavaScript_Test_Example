...
// const Pagination = dynamic(() => import("widgets/Common/Mobile/Pagination"));
const IplVideosList = ({ data }) => {
  ...
  const query_arr =  { post_type:"videos",'tags.slug':'ipl-2023' };
  const isIPL = true;
  const { loadMore, categoryData, hasMoreData } = useLoadMore(videoPriorityData, pageLimit, dataLength, query_arr, isIPL);
  return (
    <>
      ...
      <IplDataList list={categoryData} pageAds={pageAds} />
      {/* {noContent ? (
        <Pagination
          curpage={page}
