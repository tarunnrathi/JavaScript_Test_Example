...
export const getDataByRedisKey = async (queryString) =>
  await fetchUtility(`${URL.GET_REDIS}?${queryString}`);

  export const getMenu = async (isMobile = false, isAmp = false) => {
    const baseURL = URL.getURL(URL.GET_COMMON_MENU);
    let [data, getStickyFooter] = await Promise.all([
      fetchUtility(
        `${baseURL}${isAmp ? "?isAmp=true" : isMobile ? "" : "?isDesktop=true"
        }`,
        {},
        `menu_data${isMobile ? "_mobile" : isAmp ? "_amp" : "_desktop"}`,
        21600
      ),
      isAmp ? getRedisDataByKeyWithCache("msf", "KHABARN18-", false, 100) : []
    ]);
    if(isAmp) {
      let stickyFooter = getFilterMenu(getStickyFooter, isMobile, isAmp);
      data.stickyFooter = stickyFooter;
    }  
    return data;
  };
...
export const getArticleAdjacents = async ({
  ...
  const baseURL = URL.getURL(URL.GET_ARTICLE_ADJACENTS, false, customURL);
...
export const getDistricts = async () => {
  ...
  const [redisStateData, redisCityData] = await Promise.all([
    getRedisDataByKeyWithCache(REDIS_KEYS.LOCAL_STATE),
    getRedisDataByKeyWithCache(REDIS_KEYS.LOCAL_CATEGORY),
  ]);
...
export const getMiscData = async ({
  ...
  const data = {};
  if (trendingTags || catName || image || eventData) {
    const rdata = await getRedisDataByKey(REDIS_KEYS.NW_HM_DATA);
    if (trendingTags) {
      data.trendingTags = rdata["trending_tags"]
        ? rdata["trending_tags"]["default"]
          ? rdata["trending_tags"]["default"]
          : []
        : [];
    }
    ...
    if (catName && cat) {
      data.catName = rdata["categories_management"]
        ? rdata["categories_management"][cat] || cat
        : cat;
    }

    if (image) {
      data.image = rdata["image_management"]
        ? rdata["image_management"]["default"]
        : {};
    }

    if (eventData) {
      data.events = rdata["image_management"]
        ? rdata["image_management"]["events"]
        : {};
      data.eventsManagement = rdata["events_management"] || {};
    }
  }
...
export const getPriorityData = async (
  ...
  const baseURL = URL.getURL(URL.GET_PRIORITY, isCSRCall, customURL);
...
export const getWebStories = async ({
  ...
  const baseURL = URL.getURL(URL.GET_WEBSTORY);
...
export const getPageMeta = async (
  ...
  const baseURL = URL.getURL(URL.GET_PAGE_META);
  const data = await fetchUtility(
    `${baseURL}?type=category&pCategory=${
      section ? section : subsection
    }&category=${subsection ? subsection : section}${
...
export const getRedisDataByMultiKey = async (key) => {
  const baseURL = URL.getURL(URL.GET_REDIS_URL);
  const data = key
    ? await fetchUtility(`${baseURL}?key=${key}&allow_prefix=false`)
    : {};
  return data || {};
};
...
export const RhsphotoStories = async (isCSRCall = false, tag = "") => {
  const filter = tag ? { "tags.slug": tag, post_type: "photogallery" } : { post_type: "photogallery" }
...
export const RhstopStory = async (filter, id) => {
  let topStories = await getArticleList({
    count: 4,
    offset: 0,
    ...
    sortOrder: "desc",
    sortBy: "created_at",
    cache: { key: "rhs_top_stories_by_filter", ttl: 300 },
  });
  if (topStories.length <= 0) {
    topStories = await getArticleList({
      count: 6,
      offset: 0,
      fields: "story_id,display_headline,images,weburl_r,weburl",
      filter: { post_type: "text", "categories.slug": "nation" },
      sortOrder: "desc",
      sortBy: "created_at",
      cache: { key: "rhs_top_stories", ttl: 300 },
    });
  }
  if (id) topStories = topStories.filter((story) => !story.weburl.includes(id));
  return topStories;
};
...
export const getHomeTopNews = async (
  ...
  let baseURL = URL.getURL(URL.TOP_NEWS, isCSR);
...
export const getAQiData = async () => {
  const baseURL = URL.getURL(URL.GET_AQI_DATA);
  return await fetchUtility(`${baseURL}`);
};
...
export const getCricketData = async (queryString, key, ttl, lng = "hi") => {
  const url =
    lng === "en"
      ? `${URL.CRICKET_NEXT_APIURL_ENG}${queryString}`
      : `${URL.CRICKET_NEXT_APIURL}${queryString}`;
  return await fetchUtility(url, [], key, ttl);
};

export const getCricketDataDirect = async (queryString, key) =>
  await sfetchUtilityDirect(`${URL.CRICKET_NEXT_APIURL}${queryString}`, [], key);
...
export const getBlogAuthors = async (queryString) =>
  await fetchUtility(URL.getURL(`${queryString}`));
...
export const getMobileBrand = async () => {
  const baseURL = URL.getURL(URL.GET_MOBILE_BRAND);
  let tempApi = `${baseURL}`;
  return await fetchUtility(`${baseURL}`);
};
...
export const getPetrolDieselPrices = async ({ city = "", limit = 1 }) => {
  const baseURL = URL.getURL(URL.GET_PETROL_DIESEL, true);
  return await fetchUtility(
...
export const getTopVideos = async (isCSR = false) => {
  const data = await getPriorityData(
    {
      ...
      section: "top_video",
      fields:
        "video_details,local18_video,video_id,created_at,story_id,weburl_r,youtubeid,display_headline,images",
      fallback: false,
    },
    isCSR
  );
  const local18_data = await getArticleList(
    {
      filter: { ff_source: "Hyperlocal", not: "{local18_video: '' }" },
      count: 5 - data.length,
      offset: 0,
      fields:
        "video_details,video_id,created_at,story_id,weburl_r,youtubeid,display_headline,local18_video,images",
    },
    isCSR
  );
...
export const getHomePageCricketArticles = async (story_id) => {
  const baseURL = URL.getURL(URL.GET_ARTICLE_LIST);
  return await fetchUtility(`${baseURL}?filter={"story_id":[${story_id}]}`);
};
...
export const getRealtimeCitiesByCountry = async () => {   
  const baseURL = publicRuntimeConfig.engApiUrl+URL.GET_REDIS_URL+"?allow_prefix=false&key=NEWS18:getRealtimeCitiesByCountry";
  const data =  await fetchUtility(`${baseURL}`);
  return data["NEWS18:getRealtimeCitiesByCountry"];  
};
