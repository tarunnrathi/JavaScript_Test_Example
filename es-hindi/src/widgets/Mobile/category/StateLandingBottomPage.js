...
//   console.log("query ", query_arr)
      ...
      data.map((element, key) => {
        ...
        if (priorityData == 1) {
          tempObj = {
            ...
            sectionName: (element.category) ? element.category[0] : 'news',
            url: element.url,
  ...
  const getData = async () => {
    const film = (subsection == "film-review") ? ',article_data' : '';
    const NewsData = [];
              ...
              ? topRecord.map((listNews, key) => (
                  <div className="pgtbox vsp16">
                    <ul>
                      <li>
                        <figure>
                          {(listNews?.ff_source=="Hyperlocal" && listNews?.local18_video_s !='') ? <span className="nwvideoicon"></span>:""}
                          <a href={listNews.url}>
                            <LazyImage
                              width={364}
                              height={288}
                          ...
                          <div className="chmpntpnwshd">
                            <a href={listNews.url}>
                              <div className="tpall">
                                <span className="tpc">
                                  {listNews.sectionName || listNews.content_type}
                                </span>
                              </div>
                            </a>
                            <h3>
