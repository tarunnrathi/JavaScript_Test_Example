...
import { getSliderforPriority } from "api_dns/individual/Category";
...
const categoryProps = async (context, mobile, isAmp) => {
  ...
  */
  ...
  let breadCrumbArray = [];
  let tempobj = {};
  const homeText = "हिंदी न्यूज"; //'होम';
  ...
  const fields =
    "story_id,categories,subsection,intro,display_headline,created_at,headline,post_type,images,images_all_sizes,weburl,weburl_r,ff_source,gallery_count";

  let topPriorityDataList = [];

  const storyDivider = async (
    sliderNeed,
    stories = [],
    isMobile = false,
  ) => {
    ...
    return {
      ...
      rightCatThird: stories.slice(3,10),
    };
  };
  if (is_paging === 0) {
  ...
  const [
    ...
    await getArticleList({
        ...
        filter: { post_type: "videos", "categories.slug" : subsection },
        cache: { key: "local18-category-" + subsection , ttl: 300}
      }),
      await getArticleList({
        count: 11,
        offset: 0,
        ...
        filter: { "categories.slug": section },
        cache: { key: "local18-category-" + section , ttl: 300}
      }),
      getArticleList({
        count: 6,
        offset: 0,
        sortOrder: "desc", sortBy: "created_at",
        filter: { post_type: "photogallery", "categories.slug": subsection },
        fields: `headline,images,display_headline,weburl,categories,gallery_count,weburl_r`,
        cache: { key: "local18-category-p" + subsection , ttl: 300}
      }),
      getWebStories({ count: 10,sortOrder: "desc", sortBy: "created_at", filter: { categories: section }, fields: 'blog_title,web_url_r,web_url,feature_img,categories', cache: { key: "local18-webstories-p" + section , ttl: 300} }),
      getArticleList({ count: 10, offset: 0,sortOrder: "desc", sortBy: "created_at", cache: { key: "local18-shorts-p" + subsection , ttl: 300},
         filter: { "tags.slug": subsection, "nw_auto_yt_video_type": "shorts" }, fields: `story_id,display_headline,headline,images,blog_title,categories,images,weburl_r,weburl`}),
      getArticleList({ count: 6, offset: 0, sortOrder: "desc", sortBy: "created_at", cache: { key: "local18-shorts-p-money18" , ttl: 300},
        filter: { "tags.slug": "money18", /*"categories.slug": subsection*/ }, fields: `story_id,intro,display_headline,headline,images,categories,images,weburl_r,weburl`}),
      getArticleList({ count: 5, offset: 0, sortOrder: "desc", sortBy: "created_at", cache: { key: "local18-shorts-best18-"+subsection , ttl: 300},
        filter: { "agency_full.slug": "local18", "categories.slug": subsection }, fields: `story_id,intro,display_headline,headline,images,categories,images,weburl_r,weburl`}),
