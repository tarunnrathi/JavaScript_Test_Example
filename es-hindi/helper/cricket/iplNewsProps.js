...
const iplNewsProps = async (context) => {
  ...
  const pageLimit = 15;
  const paramObj = {
    pageUrl: "/cricket/ipl/news-in-hindi/",
  ...
  let [
      ...
      getMenu(isMobile),
      getArticleList({count: limit, offset:start, filter: { post_type:"text",'tags.slug':`ipl-${IPL_YEAR}` }, fields: 'id,display_headline,weburl_r,images,headline,intro,created_at'}),
      // getDistricts(),
  ...
  const SEOData =
    (seoData?.ipl_2022 || []).find(
      (seo) => seo.ipl_page_section === "ipl22_news"
    ) || {};
    let breadCrumbArray = [];
    ...
    breadCrumbArray = [
      ...
      { slug: baseURL + "cricket/ipl/news/", value: 'न्यूज़' }
    ];
  const pageSeo = {
    title: SEOData.meta_title || title,
    keywords: SEOData.meta_ipl_keyword || keywords,
    description: SEOData.meta_ipl_page_description || desc,
    canonical: publicRuntimeConfig.siteUrl + "cricket/ipl/news-in-hindi/",
    og_image:
  ...
  pageSeo.jsonLdForWebPage =
    jsonLdForWebPage(
      ...
      publicRuntimeConfig.siteUrl + "cricket/ipl/news-in-hindi/",
      null,
      false,
      false,
      false
  ...
  const pageData = {
    ...
    dataLength: latestNewsData.length || 0,
    paramObj,
    seoData: SEOData,
    showBannerInIPL,
    _1xbetData,
