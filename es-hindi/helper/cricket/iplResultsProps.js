import { getIPLResultDetails } from "includes/seo.util";
...
import { IPL_SERIES_ID, IPL_SERIES_NAME } from "includes/ipl.helper";
...
const iplResultsProps = async (context) => {
  ...
  const { title, desc, keywords } = getIPLResultDetails();
  ...
  let [
      ...
      getMenu(isMobile),
      getCricketData(`series-result/${ipl_series_id}`),
      // getDistricts(),
  ...
  (seoData?.ipl_2022 || []).find(
    (seo) => seo.ipl_page_section === "ipl22_results"
  ) || {};
  ...
  const pageSeo = {
    ...
    canonical: publicRuntimeConfig.siteUrl + "cricket/ipl/result/",
    og_image:
  ...
  pageSeo.jsonLdForWebPage =
    jsonLdForWebPage(
      ...
      publicRuntimeConfig.siteUrl + "cricket/ipl/result/",
      null,
      false,
      false,
      false
...
export default iplResultsProps;
