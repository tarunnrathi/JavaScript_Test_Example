...
const LiveMatchProps = async (context) => {
  const urlParam = context.query || "";
  const match = urlParam?.matchId || "";
  const matchId = match?.split("-")?.slice(-1).pop();
  ...
  let [
    ...
    sFetchUtilityDirect(
      `${SITE_CONfIG.PRO_KABADDI_API}2019/${matchId}_match.json`,
      {}
    )
  ]);
  ...
  pageSeo.breadCrumbArray = [
    ...
    { value: urlParam?.teama + " vs " + urlParam.teamb, slug: currentUrl }
   ];
  const pageData = {
    isMobile,
    currentUrl,
