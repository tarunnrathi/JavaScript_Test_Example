...
const Across = ({
  ...
  const fetchData = async () => {
    ...
    const result = await fetch(
      // `https://elections-v3-gcs-json.news18.com/feed/${lang}/as/results-live.json`
      `https://elections-v3-gcs-json.news18.com/${dry}feed/${lang}/as/results-live.json`
    );
    let fetchResults = await result.json();
    if (
      fetchResults &&
      fetchResults.data &&
      Array.isArray(fetchResults.data.assembly) &&
      fetchResults.data.assembly?.length
    ) {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (
      ...
      let gValue = new Glide(el, {
        ...
        gap: 0,
        slidesToScroll: 1,
        startAt: gIndex > assemblyData?.assembly?.length ? 0 : gIndex,
      });
      gValue.mount();
      setG(gValue);
    }
  ...
  const handleOC = e => {
    if (
      !assemblyData ||
      !assemblyData.assembly ||
      assemblyData.assembly.length === 0
    ) {
      window.removeEventListener('scroll', handleScroll);
      // fetchData();
    }
  ...
  const sortData = parties => {
    let sortedParties = parties
      .sort((a, b) => b.winOrLead - a.winOrLead || a.partyOrder - b.partyOrder)
      .sort((a, b) => {
              ...
              <>
                ...
                <div className={`arsltwgtsld arsltwgtsld-${instance}`}>
                  <div data-glide-el="track">
                    <ul>
                      {assemblyData?.assembly?.map((item, index) => {
                        let stateName = stateNames.fullNames[item.shortState];
                        item.parties = sortData(item?.parties);
                        return (
                          <li>
                            <a href={`/elections/analytics-center/?t=AS&s=${item?.shortState?.toUpperCase()}`}>
                              <div className="arsltwgthd">
                                <h2>
                                  ...
                                  <span>
                                    {assemblyData?.labels?.resultTally?.toUpperCase()}
                                  </span>
                                </h2>
                              </div>
                                ...
                                {item?.parties?.length > 4
                                  ? item?.parties
                                      ...
                                      ?.map((party, idx) => (
                                        <>
                                          <span
                                            key={`${party?.partyAbbr}${party?.winOrLead}`}
                                            style={{
                              ...
                              <div className="arsltwgtbtm">
                                <span>
                                  <b>{item?.resultDone}/</b>
                                  {item?.seats}{' '}
                                  {assemblyData?.labels?.seatLabel.toUpperCase()}
                                </span>
                                <a
                                  href={`/elections/${stateName}/${stateName}-assembly-election-result-key-candidates-performance/`}>
                                  {assemblyData?.labels?.keyCandidateLabel}
                                </a>
                              </div>
                            </a>
                          </li>
